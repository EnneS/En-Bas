...
class CollectibleGem 
  attr_reader :x, :y #position, peut être lu depuis l'exterieur
...
class Layer
  ...
  def generateNew()
    ...
    width_points_total = width_points + 2
    tmp_wt = width_points_total

    octaves = Array.new(oct_nbr) do |x|
      Array.new(tmp_wt) do |y|
        GenerateKeyPoint(x, amplitude / (2 ** x), 0)    
      end
      tmp_wt *= 2
    end
  end
    ...
    if oct_nbr > copy_n
      for i in copy_n..oct_nbr-1
        ...
        for j in 0..template[i].size
          octaves[i][j] = GenerateKeyPoint(x, amplitude / (2**i), layer_index)
        end
      end
    end
  end
...
class Map
  ...
  def generate(filename, width_points, h, sea_lvl, wave_length_pow, nb_oct, amplitude)
    layers = Array.new(3){Array.new(w)}
    layers[0] = Layer.new(Tiles::Grass)
    data = Array.new(w) do |x|

      Array.new(h) do |y|
        for i in (0..layers.size).reverse
          l = 0.0
          for j in (0..layers[i].octaves.size)
            twl = w / layers[i].octaves[j].size
            a = max(0, x - 1) / twl
            b = min(a + 1, layers[i].octaves[j].size - 1)
            x = max(0, x - 1) % twl / twl.to_f
            l += interpolate(layers[i].octaves[j][a], layers[i].octaves[j][b], )
          end
        end
      end
    end
